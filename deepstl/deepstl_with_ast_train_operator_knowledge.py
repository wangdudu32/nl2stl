...
STRUCTURE_PRESERVATION_RULES = """Translation principles:
1. Copy every signal, parameter, and identifier exactly from the requirement. Do not invent aliases
   or include descriptive words in a name. A signal or parameter must not be encoded as a string
   constant.
2. Build the formula by semantic scope from outside to inside. Add an AST operator only for a
   distinct semantic operation expressed by the requirement.
3. Treat repeated or synonymous temporal wording as one operation when it refers to the same
   evaluation frame. Use separate or nested temporal nodes only for genuinely distinct scopes or
   intervals.
4. Preserve complete operands, boolean grouping, temporal scope, and explicit negation. Do not
   simplify, distribute, dualize, or replace the expressed structure with an equivalent form.
5. Negation applies to the complete constituent being denied. rise(phi) and fall(phi) represent
   false-to-true and true-to-false changes of phi; preserve phi and do not exchange an edge with its
   complemented form.
6. Use only relations and fields allowed by the JSON Schema. Express a not-equal relation as boolean
   not applied to equality.
7. Copy finite interval bounds from the requirement and use "inf" as the only symbolic unbounded
   endpoint. Never invent symbolic time endpoints.
"""

FINAL_SELF_CHECK = """Before answering, silently verify identifier fidelity, one-to-one semantic
operator mapping, operand and negation scope, interval representation, and JSON Schema validity.
Return only the AST JSON object."""

SCHEMA_ANNOTATION_KEYS = {"description", "$comment", "examples", "title"}
...
OUTPUT_FILE = ROOT / "result/deepstl_with_ast_train_operator_knowledge_deepseek_v4_pro_result.txt"
FAIL_TIMES_FILE = ROOT / "tmp/deepstl_with_ast_train_operator_knowledge_fail_times.txt"
AST_INTERMEDIATE_FILE = ROOT / "result/ast_intermediate/deepstl_with_ast_train_operator_knowledge.jsonl"
...
def strip_schema_annotations(value: Any) -> Any:
    if isinstance(value, dict):
        return {
            key: strip_schema_annotations(item)
...
def load_schema() -> tuple[str, Draft202012Validator]:
    ...
    prompt_schema = strip_schema_annotations(schema)
    prompt_schema_text = json.dumps(prompt_schema, ensure_ascii=False, separators=(",", ":"))
    return prompt_schema_text, Draft202012Validator(schema)
...
def build_system_prompt(schema_text: str, operator_knowledge: str) -> str:
...
- Use the STL operator knowledge as semantic and structural guidance for choosing the correct
  schema operator and preserving its intended scope.
- If the STL operator knowledge conflicts with the JSON Schema, the JSON Schema takes precedence.
...
Preserve identifiers and semantic scope, and do not introduce unexpressed or equivalent operators.
"""
    if error is None:
        return prompt
    return f"""{prompt}
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Translate DeepSTL requirements with resume support.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    system_prompt = build_system_prompt(schema_text, operator_knowledge)
