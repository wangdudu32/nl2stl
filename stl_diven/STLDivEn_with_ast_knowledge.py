...
INPUT_CSV = ROOT / "dataset/stl_diven_sample_300_v1.csv"
SCHEMA_FILE = ROOT / "knowledge_base/ast_schema.txt"
DEEPSTL_KNOWLEDGE_FILE = ROOT / "knowledge_base/deepstl_knowledge.txt"
OUTPUT_FILE = ROOT / "result/STLDivEn_with_ast_knowledge_deepseek_v4_pro_result.txt"
FAIL_TIMES_FILE = ROOT / "tmp/STLDivEn_with_ast_knowledge_fail_times.txt"
AST_INTERMEDIATE_FILE = ROOT / "result/ast_intermediate/STLDivEn_with_ast_knowledge.jsonl"
...
def load_deepstl_knowledge() -> str:
    return DEEPSTL_KNOWLEDGE_FILE.read_text(encoding="utf-8").strip()


def build_system_prompt(schema_text: str, deepstl_knowledge: str) -> str:
...
- Use the DeepSTL knowledge only as semantic and structural guidance for choosing
  the correct formula structure, temporal operators, and predicates.
- If the DeepSTL knowledge conflicts with the JSON Schema, the JSON Schema takes
  precedence.
- Use operator names exactly as specified by the JSON Schema enum values, not
  symbolic aliases.
- Do not copy knowledge section names, type labels, or template placeholders such
  as "<TRIGGER>" into the AST.
...
def main() -> None:
    ...
    deepstl_knowledge = load_deepstl_knowledge()
    system_prompt = build_system_prompt(schema_text, deepstl_knowledge)
